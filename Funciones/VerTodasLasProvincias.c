...
int main()
{
    ...
    verTodasLasProvincias(RegistroMatricula, RegistroTrayectoria);
    return 0;
}
    ...
    while (auxM != NULL) {
        if (strcmp(auxM->provincia, provinciaBuscada) == 0) {
            printf("%-25s | %-10s | %-6d | %-6d | %-6d | %-6d | %-6d | %-6d | "
                   ...
                   auxM->reg.matriculas[0], auxM->reg.matriculas[1],
                   auxM->reg.matriculas[2], auxM->reg.matriculas[3],
                   auxM->reg.matriculas[4], auxM->reg.matriculas[5],
                   auxM->id);
            encontradoM = 1;
        }
        auxM = auxM->sig;
