                        ...
                        Matricula **tempM, NodoTrayectoria **tempT)
{
    ...
    NodoTrayectoria *lista_a_filtrar_T = (*tempT != NULL) ? *tempT : RegistroTrayectoria;
    Matricula *lista_a_filtrar_M = (*tempM != NULL) ? *tempM : RegistroMatricula;

    NodoTrayectoria *nuevo_tempT = NULL;
    NodoTrayectoria *cola_tempT = NULL;
    Matricula *nuevo_tempM = NULL;
    Matricula *cola_tempM = NULL;

    if (*tempT != NULL) { /* LiberarListaTrayectoria(tempT); */ *tempT = NULL; } 
    if (*tempM != NULL) { /* LiberarListaMatricula(tempM); */ *tempM = NULL; }
    ...
    while (aux != NULL)
    {
        ...
        
        {
            ...
            for (int i = 0; aux->provincia[i] != '\0' && j < 24; i++) {
                if (aux->provincia[i] != ' ' && aux->provincia[i] != '\t') {
                    aux_provincia_normalizada[j] = tolower((unsigned char)aux->provincia[i]);
                    j++;
                }
            }
        ...
        {
            bandera = 1;
            printf("%-15s | %-10s | %-6d | %-6d | %-6d | %-6d | %-6d | %-6d | %-6d | "
                   "%-6d | %-6d | %-6d | %-6d | %-6d | %-9d | %d\n",
                   aux->provincia, aux->sector, aux->trayectoria.promovidos[0],
                   aux->trayectoria.promovidos[1], aux->trayectoria.promovidos[2],
                   aux->trayectoria.promovidos[3], aux->trayectoria.promovidos[4],
        ...
        char aux2_provincia_normalizada[25];
        {
            ...
            for (int i = 0; aux2->provincia[i] != '\0' && j < 24; i++) {
                if (aux2->provincia[i] != ' ' && aux2->provincia[i] != '\t') {
                    aux2_provincia_normalizada[j] = tolower((unsigned char)aux2->provincia[i]);
                    j++;
                }
            }
    ...
    while (auxT != NULL)
    {
        if (auxT->id == anio)
        {
            ...
            printf("%-15s | %-10s | %-6d | %-6d | %-6d | %-6d | %-6d | %-6d | %-6d | "
                   ...
                   auxT->trayectoria.nopromovidos[1], auxT->trayectoria.nopromovidos[2],
                   auxT->trayectoria.nopromovidos[3], auxT->trayectoria.nopromovidos[4],
                   ...
                   Matricula **tempM, NodoTrayectoria **tempT)
{
    ...
    while (aux != NULL)
    {
        ...
        {
            int j = 0;
            for (int i = 0; aux->sector[i] != '\0' && j < 24; i++) {
...

        {
            ...
            for (int i = 0; aux2->sector[i] != '\0' && j < 24; i++) {
                if (aux2->sector[i] != ' ' && aux2->sector[i] != '\t') {
                    aux2_sector_normalizado[j] = tolower((unsigned char)aux2->sector[i]);
                    j++;
                }
            }
